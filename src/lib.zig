...
pub fn linkBearSSL(comptime path_prefix: []const u8, module: *std.build.LibExeObjStep, target: std.zig.CrossTarget) void {
    ...
    module.addIncludePath(path_prefix ++ "/BearSSL/inc");
    module.addIncludePath(path_prefix ++ "/BearSSL/src");
...
pub const Client = struct {
    ...
    pub fn reset(self: *Self, host: [:0]const u8, resumeSession: bool) !void {
        const err = c.br_ssl_client_reset(&self.client, host.ptr, if (resumeSession) @as(c_int, 1) else 0);
        if (err < 0)
